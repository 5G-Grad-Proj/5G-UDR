pipeline {
    ...
    stages {
        ...
        stage('Pulling base image from Dockerhub') {
            steps {
                    sh 'docker pull gradproj/udr-base'
            }
        }
        ...
        stage('docker build') {
            steps {
                sh(script: """
                    docker images -a
                    docker build -t gradproj/5g-udr:latest . 
                    docker images -a
                """)
            }
        }
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'trivy image gradproj/5g-udr --output trivy-report.json'
            }
        }
        stage('Pushing to Dockerhub') {
            steps {
                sh 'docker push gradproj/5g-udr:latest'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker rmi gradproj/5g-udr'
            sh 'docker rmi gradproj/udr-base'
        }
    }
}